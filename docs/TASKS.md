...
- **2026-09-08 — The drift is now prevented mechanically, not by discipline.** `npm run check:i18n` fails the build when a key the app asks for is not seeded by any migration, and it runs inside `verify`. Keys built from a variable cannot be read statically, so every such site is declared in the script with the values it can take; an **undeclared** dynamic site fails the check rather than passing silently, because an unknown pattern is a hole in the guard rather than an absence of keys. Verified the way the write-path guard was: a key was planted, the check named the exact key and file and exited 1, and the plant was removed.

- **2026-09-08 — Caveat these files do NOT fix: the remote migration *history* is still divergent.** The 12 local files reproduce the database on a **fresh** project. They will not `supabase db push` cleanly against the existing project, because that history was written through the MCP `apply_migration` tool under different names (43 entries, e.g. `foundation_schema_and_rls`), and `0001` would try to `create table` over tables that already exist. Reconciling that means rewriting the remote migration ledger, which is riskier than the problem and was not attempted. Stand up new environments from these files; leave the existing project as it is.
